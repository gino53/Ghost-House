...
gui.add({SoundOn: isSoundOn}, 'SoundOn').onChange(function(value) {
    if (value) {
        audio.play();
    } else {
        audio.pause();
...
const tick = () =>
{
    ...
    ghost1.position.x = Math.cos(ghost1Angle) * (3.5 + Math.sin(elapsedTime * 0.72))
    ghost1.position.z = Math.sin(ghost1Angle) * (3.5 + Math.sin(elapsedTime * 0.5))
    ghost1.position.y = Math.sin(elapsedTime * 3)

    const ghost2Angle = - elapsedTime * 0.32
    ghost2.position.x = Math.cos(ghost2Angle) * (6 + Math.sin(elapsedTime * 0.32))
    ghost2.position.z = Math.sin(ghost2Angle) * (6 + Math.sin(elapsedTime * 0.6))
    ghost2.position.y = Math.sin(elapsedTime * 4) + Math.sin(elapsedTime * 2.5)

    const ghost3Angle = - elapsedTime * 0.18
    ghost3.position.x = Math.cos(ghost3Angle) * (8 + Math.sin(elapsedTime * 0.32))
